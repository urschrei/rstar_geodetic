...
/// [`rstar::Point`] (`DIMENSIONS = 3`), which makes [`rstar::AABB<UnitVec>`] a
...
pub struct UnitVec(
    ...
    /// corner type for [`rstar::AABB<UnitVec>`], where instances are general 3D
    /// bounds and are not unit length.
    pub [f64; 3],
);

impl rstar::Point for UnitVec {
    type Scalar = f64;
...
/// [`distance_2`](rstar::PointDistance::distance_2), in the same squared-chord units the
/// [`AABB<UnitVec>`](rstar::AABB) envelope uses; convert to metres with
